...
export default function DoctorSidebar({ activePage, collapsed, onToggleCollapse }: DoctorSidebarProps) {
  ...
  const navItems = [
    { icon: <Calendar className="h-5 w-5" />, label: "Calendar", path: "/doctor-dashboard/scheduling", key: "calendar" },
    { icon: <User className="h-5 w-5" />, label: "Patients", path: "/doctor-dashboard/patients", key: "patients" },
    { icon: <MessageSquare className="h-5 w-5" />, label: "Messages", path: "/doctor-dashboard/messages", key: "messages" },
  ]
      ...
      <div className="flex flex-col gap-4 items-center mt-6 px-2">
        {navItems.map(({ icon, label, path, key }) => (
          <Button
            ...
            className={`w-full justify-${collapsed ? "center" : "start"} px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200 ${
              activePage === key
